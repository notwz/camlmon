...
let ps_move_test (name : string) state move expected_output : test = 
  name >:: fun _ -> 
    assert_equal expected_output (use_move state move) 

let ps_damaged_test (name : string) state damage expected_output: test = 
  name >:: fun _ -> 
    assert_equal expected_output (damaged state damage)
let p_state_tests = [ 
  ...
  ps_valid_move_test "valid move on test state" test_ps_0 "shove" false; 
  ps_move_test "Using cut once" test_ps_0 "cut" test_ps_1; 
  ps_damaged_test "Pokemon received 2 dmg : " test_ps_0 2 test_ps_damaged; 
  ps_damaged_test "Pokemon received 2 dmg : " test_ps_1 2 test_ps_damaged; 
