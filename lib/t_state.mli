...
val move_down : t -> t
(** [move_down t] is the trainer state after the trainer [t] is moved
    down one square. Raises [OutOfBoundsMove pos] with the current
    position if move places trainer out of bound. *)

val move_left : t -> t
(** [move_left t] is the trainer state after the trainer [t] is moved
    left one square. Raises [OutOfBoundsMove pos] with the current
    position if move places trainer out of bound. *)

val move_right : t -> t
(** [move_right t] is the trainer state after the trainer [t] is moved
    right one square. Raises [OutOfBoundsMove pos] with the current
    position if move places trainer out of bound. *)
